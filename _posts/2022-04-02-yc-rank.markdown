...
YCRank examines abbreviated descriptions like the one below to predict a logit score, which is then used to sort against other companies. I also employed a data augmentation trick where I generated a small number of permutations of the input string based on re-ordering the metadata. For example, presenting the description of the company before its name.
...
- It will take me some more time to understand if there were any interesting false positives / negatives in this ranking, so I'd appreciate if my readers can point out anything interesting.

Please don't take it personally if your company ranks low on this list, this is more of a proof-of-concept of how to train models to reflect one person's opinions. You can take solace in that I'm not a particularly great investor - if you invert this list you might actually do quite well.
